...
DROP FUNCTION IF EXISTS public.get_inventory_stock_summary(uuid, boolean);

-- Recreate with the new return type that includes in_process_quantity
CREATE OR REPLACE FUNCTION public.get_inventory_stock_summary(p_organization_id uuid, p_include_zero_stock boolean DEFAULT false)
 RETURNS TABLE(item_id character varying, item_description text, division_id uuid, division_name text, quantity_available numeric, in_process_quantity numeric, uom character varying, last_updated_on timestamp with time zone, item_group_name text, classification character varying, sub_classification character varying)
 LANGUAGE plpgsql
 STABLE
AS $function$
BEGIN
    ...
    WITH aggregated_stock AS (
        SELECT
            ...
            SUM(
                CASE
                    WHEN inv.transaction_type IN ('PO_RECEIVE', 'ADJUSTMENT_IN') THEN inv.available_quantity
                    WHEN inv.transaction_type IN ('SALES_ORDER', 'ADJUSTMENT_OUT') THEN -inv.available_quantity
                    WHEN inv.transaction_type = 'TRANSFER' THEN inv.available_quantity
                    ELSE 0
                END
            ) AS calculated_quantity,
            SUM(inv.in_process_quantity) AS calculated_in_process_quantity,
    ...
    SELECT
        ...
        agg.calculated_in_process_quantity::numeric,
